...
export class CookupClient {
  ...
  public async getCookups(userId: number, recipeId: number): Promise<Cookup[]> {
    return new Promise<Cookup[]>((resolve, reject) => {
      this.client.getCookupCollection(
        userId.toString(), [], recipeId.toString(), []
      )
        .then((response) => {
          const cookups = this.viewModelConverter.convertCookups(response.data['hydra:member']);
          resolve(cookups);
        })
        .catch((e) => {
          logAxiosError(e);
  ...
  public async deleteCookup(cookupId: number): Promise<void> {
    return new Promise<void>((resolve, reject) => {
      this.client.deleteCookupItem(cookupId.toString())
        .then(() => resolve())
        .catch((e) => {
          logAxiosError(e);
          reject(e)
