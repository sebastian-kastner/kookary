export const INGREDIENTS_ENDPOINT = "/api/ingredients";
export const INGREDIENT_CATEGORY_ENDPOINT = "/api/ingredient_categories";
export const RECIPES_ENDPOINT = "/api/recipes";
export const TAGS_ENDPOINT = "/api/tags";
export const MEDIA_OBJECTS_ENDPOINT = "/api/media_objects";
export const USER_ENDPOINT = "/api/users";
export const COOKUP_ENDPOINT = "/api/cookups";
export const SHOPPING_ITEM_ENDPOINT = "/api/shopping_item";
export const USER_RECIPE_FAVOURITES = "/api/user_recipe_favourites";

export const ME_ENDPOINT = "/api/me";
export const AUTH_TOKEN_ENDPOINT = "/api/auth_token";
export const REFRESH_TOKEN_ENDPOINT = "/api/refresh_token";
export const SEASONALITY_UPDATE_ENDPOINT = "/api/update_seasonality_scores";
